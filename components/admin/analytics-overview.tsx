...
import { Activity, Eye, MousePointerClick, Radio, RefreshCw, ShoppingCart, WalletCards, type LucideIcon } from 'lucide-react';
...
type AnalyticsData = {
  ...
  sources: Slice[];
  platforms: Slice[];
  live: Array<{ sessionId: string; lastSeenAt: string; path: string; sourceType: string; sourcePlatform: string; city: string; region: string; country: string; latitude: number | null; longitude: number | null; device: string; browser: string; lastEvent: string }>;
  recent: Array<{ name: string; label: string; at: string; city: string; platform: string; device: string }>;
};

const empty: AnalyticsData = { updatedAt: '', kpis: { uniqueVisitors: 0, pageViews: 0, cartOpens: 0, checkoutStarts: 0, purchases: 0, revenue: 0, activeNow: 0 }, funnel: [], sources: [], platforms: [], live: [], recent: [] };
  ...
  return (
    <div className={`admin-overview${loading ? ' is-loading' : ''}`} aria-busy={loading}>
      ...
      <section className="analytics-live-grid">
        ...
        <article className="analytics-card analytics-activity-card"><div className="analytics-card-heading"><div><span>ÚLTIMAS AÇÕES</span><h2>Atividade em tempo real</h2></div><b><Radio /> AO VIVO</b></div><div className="analytics-activity-list">{data.recent.length ? data.recent.map((item, index) => <div key={`${item.at}-${index}`}><i /><p><strong>{item.label}</strong><span>{item.city} · {item.platform}</span><small>{item.device} · {new Date(item.at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</small></p></div>) : <div className="analytics-activity-empty"><Activity /><strong>Nenhuma ação no período</strong><span>Os acessos reais aparecerão aqui, sem dados de demonstração.</span></div>}</div></article>
      </section>
      ...
      <p className="analytics-footnote">Atualizado {data.updatedAt ? new Date(data.updatedAt).toLocaleString('pt-BR') : 'agora'}. A localização é aproximada e nenhum endereço IP é armazenado.</p>
    </div>
  );
}
